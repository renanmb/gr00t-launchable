...
# Wait for the network and base directory to be available before doing anything
echo "Checking system readiness at $(date)..." >> "$OUTPUT_LOG"

MAX_RETRIES=10
RETRY_COUNT=0

while [ ! -d "$BASE_DIR" ] || [ $RETRY_COUNT -lt 1 ]; do
    ...
    # Check if we can actually reach the internet (required for apt/pip installs)
